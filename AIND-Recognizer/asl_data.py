...
if __name__ == '__main__':
    ...
    features = ['right-y', 'right-x', 'left-y','left-y']

    mean_right_x = asl.df['speaker'].map(df_means['right-x'])
    std_right_x = asl.df['speaker'].map(df_std['right-x'])

    def addNormFeatures(feat):
        nf, f = feat
        mean_f = asl.df['speaker'].map(df_means[f])
        std_f = asl.df['speaker'].map(df_std[f])
    
        asl.df[nf] = (asl.df[f] - mean_f) / std_f
    
    #[addNormFeatures(f) for f in zip(features_norm, features)]
    
    features_polar = ['polar-rr', 'polar-rtheta', 'polar-lr', 'polar-ltheta']
    features_grnd = ['grnd-rx', 'grnd-ry', 'grnd-lx','grnd-ly']

    def addPolarFeatures(feat_p, feat_d):
        pr = feat_p[0]
        pt = feat_p[1]
        dx = feat_d[0]
        dy = feat_d[1]
        
        asl.df[pr] = np.sqrt(pow(asl.df[dx],2)+pow(asl.df[dy],2))
        asl.df[pt] = np.arctan2(asl.df[dx], asl.df[dy])
        

    addPolarFeatures(features_polar[:2], features_grnd[:2])
    addPolarFeatures(features_polar[2:], features_grnd[2:])

    def addDeltaFeatures(feat):
        nf, f = feat
        asl.df[nf] = asl.df[f].diff()
    
    features_delta = ['delta-rx', 'delta-ry', 'delta-lx', 'delta-ly']
    [addDeltaFeatures(f) for f in zip(features_norm, features)]
    asl.df.head()
