...
class PlanningGraph():
    ...
    def add_action_level(self, level: int):
        ...
        pos_states=[s.symbol for s in self.s_levels[level] if s.is_pos]
        neg_states=[s.symbol for s in self.s_levels[level] if not s.is_pos]
        
        pos_cond = lambda a: (len(a.precond_pos) > 0 and set(a.precond_pos).issubset(pos_states)) 
        neg_cond = lambda a: (len(a.precond_neg) > 0 and set(a.precond_neg).issubset(neg_states))     

        act = [a for a in self.all_actions if pos_cond(a) or neg_cond(a)]
        self.a_levels.append(set())
        for a in act:
            action_node = PgNode_a(a)      
            action_node.precond_s_nodes = [x for x in self.s_levels[level] if x.symbol in a.precond_pos or x.symbol in a.precond_neg]
            action_node.effect_s_nodes = a.effect_add
    ...
    def add_literal_level(self, level):
        ...
        for a in self.a_levels[level-1]:
            s_l = [PgNode_s(x.symbol, x.is_pos) for x in a.effnodes]
            self.s_levels[level] = self.s_levels[level] | (set(s_l)) 
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        '''
        return any([x for x in node_a1.effnodes if x.symbol in [y.symbol for y in node_a2.effnodes if y.is_pos != x.is_pos]])
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        has_interference = lambda n_c_1, n_c_2: any([x in [y for y in n_c_1] for x in n_c_2])        

        pos_m = any([x in [y for y in node_a2.action.effect_add] for x in node_a1.action.precond_neg])
        neg_m = any([x in [y for y in node_a2.action.effect_rem] for x in node_a1.action.precond_pos])

        return (has_interference(node_a2.action.effect_add, node_a1.action.precond_neg) or
                has_interference(node_a2.action.effect_rem, node_a1.action.precond_pos) or 
                has_interference(node_a1.action.effect_add, node_a2.action.precond_neg) or
                has_interference(node_a1.action.effect_rem, node_a2.action.precond_pos))
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        '''
        is_mutex = False
        for pc in node_a1.parents: 
            is_mutex = any([x for x in pc.mutex if any([y for y in node_a1.parents if x in y.mutex])])
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        :return: bool
        '''
        return all([PgNode.is_mutex(x,y) for x in node_s1.parents for y in node_s2.parents])
    ...
    def h_levelsum(self) -> int:
        ...
        for g in self.problem.goal:
            ...
            for sl in self.s_levels:
                l = l + 1
                if g in [s.symbol for s in sl if s.is_pos]:
                    level_sum = level_sum + l      
                    break;
            #l = 0
            #try:
            #    l = next((ind for ind, s in enumerate(self.s_levels) if g in [x.symbol for x in s if x.is_pos]))
